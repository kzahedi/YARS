...
- [ ] **Step 2b (AMENDED 2026-07-07 — Stage 0 found 6 real order-lossy configs, one CI-exercised): `#children` ordered representation, both sides.** Reordering XML sources is NOT safe in general (hexapod_ralf/muscle_tcpip interleave actuators/sensors whose INDEX ORDER is external controllers' I/O binding). Instead: (a) upgrade `XmlToJson` — when non-contiguous same-tag siblings are detected under a parent, emit that parent's children as `"#children": [ {"#tag": "<name>", ...attrs/children...}, ... ]` preserving document order, instead of throwing (keep the throw for text content); update the `InterleavedSameTagSiblingsFailLoudly` gtest into an `InterleavedSameTagSiblingsBecomeOrderedChildren` test plus a round-trip ordering assertion; (b) the JSON reader consumes `#children` by emitting events in array order with `#tag` as the element name; (c) empty `EXPECTED_FAIL` in scripts/convert-corpus.sh — all 29 configs must convert after this; (d) round-trip verification for hexapod_logging.xml (CI corpus) must be bit-exact CSV as with everything else.

...
void main() {
    ...
    // Perspective divide. For our orthographic shadow camera w == 1,
    // but stay general so the same shader handles a perspective light
    // if we add one later.
    vec2 shadowUV = shadowClipPos.xy / shadowClipPos.w;

    // Out-of-frustum: no shadow (border-colour sample). Guard.
    float occluder = 1.0;
